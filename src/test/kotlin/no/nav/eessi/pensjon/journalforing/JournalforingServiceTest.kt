...
internal class JournalforingServiceTest : JournalforingServiceBase() {
    ...
    fun `Sendt gyldig Sed R005 på R_BUC_02`() {
        ...
        journalforingService.journalfor(
            ...
            SaksInfoSamlet(
                sakInformasjonFraPesys = SakInformasjon(sakId = "12345", sakType = ALDER, sakStatus = LOPENDE),
                saktypeFraSed = ALDER
            ),
            identifisertePersoner = 1,
            navAnsattInfo = null,
    ...
    fun `Gitt at saksbhandler oppretter en P2100 med NORGE som SAKSEIER så skal SEDen journalføres maskinelt`() {
        ...
        journalforingService.journalfor(
            ...
            identifisertPerson,
            LEALAUS_KAKE.getBirthDate(),
            SaksInfoSamlet(sakInformasjonFraPesys = sakInformasjon, saktypeFraSed = GJENLEV),
            identifisertePersoner = 2,
            navAnsattInfo = null,
            currentSed = SED(type = SedType.P2100)
        )
    ...
    fun `Gitt at saksbhandler oppretter en P2100 med NORGE som DELTAKER så skal SEDen journalføres maskinelt på gjenlevnde`() {
        ...
        journalforingService.journalfor(
            ...
            LEALAUS_KAKE.getBirthDate(),
            SaksInfoSamlet(sakInformasjonFraPesys = saksInfo, saktypeFraSed = GJENLEV),
            identifisertePersoner = 2,
            navAnsattInfo = null,
            currentSed = SED(type = SedType.P2100)
        )
    ...
    fun `Gitt at vi mottar en P_BUC_02 med kjent aktørid Når det finnes kun en sakstype fra bestemsak`() {
        ...
        journalforingService.journalfor(
            ...
            SaksInfoSamlet(sakInformasjonFraPesys = saksInfo, saktypeFraSed = BARNEP),
            identifisertePersoner = 2,
            navAnsattInfo = null,
            currentSed = SED(type = SedType.P2100)
        )
    ...
    fun `Gitt en P12000 i P_BUC_08 med tema ufoere så skal tema bli uforep`() {
        val p12000 = P12000(type = SedType.P12000, pensjonP12000 = P12000Pensjon(pensjoninfo = listOf(Pensjoninfo(betalingsdetaljer = listOf(Betalingsdetaljer(pensjonstype = "02"))))))
        val mockedSedhendelse = mockk<SedHendelse>(relaxUnitFun = true).apply {
            every { rinaSakId } returns RINADOK_ID
    ...
    fun `Gitt en P12000 i P_BUC_08 med tema gjenlevende så skal tema bli PEN`() {
        val p12000 = P12000(type = SedType.P12000, pensjonP12000 = P12000Pensjon(pensjoninfo = listOf(Pensjoninfo(betalingsdetaljer = listOf(Betalingsdetaljer(pensjonstype = "03"))))))
        val mockedSedhendelse = mockk<SedHendelse>(relaxUnitFun = true).apply {
            every { rinaSakId } returns RINADOK_ID
            every { bucType } returns P_BUC_08
            every { sedType } returns SedType.P12000
    ...
    private fun saksInfoSamlet(sakType: SakType) = SaksInfoSamlet(
        "321654",
        SakInformasjon("321654", sakType, LOPENDE),
