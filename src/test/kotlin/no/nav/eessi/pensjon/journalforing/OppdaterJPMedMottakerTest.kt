...
class OppdaterJPMedMottakerTest {
    ...
    fun `skal oppdatere alle som ligger `() {
        ...
        verify(exactly = 3) { journalpostKlient.oppdaterJournalpostMedMottaker(any(), avsenderMottker(deltakere().first().organisation!!)) }
    }
    ...
    fun `skal hoppe over allerede lagret filer`() {
        ...
        verify(exactly = 2) { euxService.hentDeltakereForBuc(any()) }
        verify(exactly = 2) { journalpostKlient.oppdaterJournalpostMedMottaker(any(), any()) }
    }
