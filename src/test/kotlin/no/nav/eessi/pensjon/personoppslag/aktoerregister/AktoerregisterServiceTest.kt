...
class AktoerregisterServiceTest {
    ...
    fun `should throw runtimeexception when 403-forbidden is returned`() {
        whenever(mockrestTemplate.exchange(any<String>(), any(), any<HttpEntity<Unit>>(), eq(String::class.java))).thenThrow(HttpClientErrorException(HttpStatus.FORBIDDEN))
        ...
        assertEquals("Received 403 FORBIDDEN Forbidden from aktørregisteret", arre.message!!)
    }
